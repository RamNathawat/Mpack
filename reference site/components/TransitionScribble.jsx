...
export default function TransitionScribble() {
    useEffect(() => {
        const logoMpackClickable = document.querySelector('.logo-mpack');
        const transitionScribblePath = document.querySelector('.transition-scribble__svg path');
        const transitionScribbleSvg = document.querySelector('.transition-scribble__svg');

        if (!logoMpackClickable || !transitionScribblePath || !transitionScribbleSvg) return;
        ...
        const runScribbleAnimation = (e) => {
            ...
            if (!transitionLogo) {
                ...
                const svgClone = document.querySelector('.logo-mpack').cloneNode(true);
        ...
        logoMpackClickable.addEventListener('click', runScribbleAnimation);

        // Auto-run on load
        const timer = setTimeout(() => runScribbleAnimation(null), 100);

        return () => {
            logoMpackClickable.removeEventListener('click', runScribbleAnimation);
            clearTimeout(timer);
        };
    }, []);
